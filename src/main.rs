...
fn filter(dir: &Dir, size: u64) -> bool {
    ...
    if is_filter == false {
        debug!("{:?}'s size: {}g", dir.path.as_os_str(), dir.g_size);
    }
    return true;
}
...
fn match_command() -> (PathBuf, u64) {
    let matches = Command::new("pacman")
        .arg(Arg::new("path").short('p').default_value(".").takes_value(true))
        .arg(Arg::new("size").short('s').default_value("5").takes_value(true))
        .get_matches();
...
async fn init_dir(mut dir: Dir) -> Result<Dir> {
    ...
    let mut read_dir = read_dir(path.as_path()).await.context(anyhow!("读取文件夹[{:?}]失败", path.as_path()))?;
        ...
        while let Ok(Some(sub_dir)) = read_dir.next_entry().await {
            ...
            if let Ok(metadata) = sub_dir.metadata().await {
                if metadata.is_file() {
                    ...
                    // dbg!("{:?}", sub_dir.path());
                    dir_res.push(tokio::spawn(async move {
                        init_dir_v2(sub_dir.path()).await
                    }))
                    ;
                }
            } else {
...
impl File {
    pub fn new(name: impl Into<OsString>,
               path: impl Into<PathBuf>,
               size: u64) -> Self {
        let m_size = size / M_SIZE;
        Self {
            name: name.into(), path: path.into(), size, m_size
        }
    }
    #[inline]
...
impl Dir {
    pub fn new(name: impl Into<OsString>,
        ...
        Self {
            name: name.into(), path: path.into(), files: Vec::default(), dirs: Vec::default(), size: 0, m_size: 0, g_size: 0
        }
    }
...
fn test() {
    ...
    println!("{} {:?}", metadata.file_attributes(), metadata.permissions());
}